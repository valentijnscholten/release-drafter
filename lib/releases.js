...
module.exports.findReleases = async ({ ref, context, config, previousVersion = undefined }) => {
  ...
  log({ context, message: `previousVersion ${previousVersion.tag_name}` })
  if (previousVersion) {
  ...
  const lastRelease = previousVersion
    ? sortedPublishedReleases.find((r) => r.tag_name == previousVersion)
