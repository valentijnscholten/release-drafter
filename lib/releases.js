...
module.exports.findReleases = async ({ ref, context, config, inputLastRelease = undefined }) => {
  ...
  log({ context, message: `inputLastRelease ${inputLastRelease.tag_name}` })
  if (inputLastRelease) {
    log({ context, message: `if: True` })
  }
  ...
  const lastRelease = inputLastRelease
    ? sortedPublishedReleases.find((r) => r.tag_name == inputLastRelease)
  ...
  if (lastRelease) {
    ...
    message = inputLastRelease
      ? `No last release matching ${inputLastRelease} found`
      : `No last release found`
    log({ context, message: message })
  }
